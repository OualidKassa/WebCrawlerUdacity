...
final class WordCounts {
  ...
  static Map<String, Integer> sort(Map<String, Integer> wordCounts, int popularWordCount) {
    ...
    sortedCounts = new PriorityQueue<>(wordCounts.size(), new WordCountComparator());
    sortedCounts.addAll(wordCounts.entrySet());

    return IntStream.range(0, Math.min(popularWordCount, wordCounts.size()))
            .mapToObj(i -> sortedCounts.poll())
